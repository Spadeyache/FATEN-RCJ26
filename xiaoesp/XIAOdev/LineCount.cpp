...
static uint16_t   s_dbgArcBlackCount = 0;
static bool       s_dbgArcBlackSaturated = false;
static bool       s_dbgHasArc = false;
static uint8_t    s_dbgArc[10] = {};
static bool       s_cfgDirty = false;
static bool       s_cfgValid = false;
static uint32_t   s_cfgLastSentMs = 0;
static uint8_t    s_cfgArc[10] = {};
static uint8_t    s_cfgSilver[4] = {};
static uint8_t    s_cfgColor[3] = {};
static uint8_t    s_cfgHamWidthMin = 0;
...
void lc_storeStreamGuides(uint8_t topX, uint8_t topY,
    ...
    const uint8_t arc[10] = {
        topX, topY, leftX, leftY, rightX, rightY,
        bottomLeftX, bottomLeftY, bottomRightX, bottomRightY
    };
    const uint8_t silver[4] = { silverLeftX, silverRightX, silverY0, silverY1 };
    const uint8_t color[3] = { colorX0, colorX1, colorY };

    bool changed = !s_cfgValid;
    for (uint8_t i = 0; i < 10 && !changed; i++) changed = (s_cfgArc[i] != arc[i]);
    for (uint8_t i = 0; i < 4 && !changed; i++) changed = (s_cfgSilver[i] != silver[i]);
    for (uint8_t i = 0; i < 3 && !changed; i++) changed = (s_cfgColor[i] != color[i]);
    if (!changed) changed = (s_cfgHamWidthMin != hamidashiWidthMin);
    if (!changed) return;

    for (uint8_t i = 0; i < 10; i++) s_cfgArc[i] = arc[i];
    for (uint8_t i = 0; i < 4; i++) s_cfgSilver[i] = silver[i];
    for (uint8_t i = 0; i < 3; i++) s_cfgColor[i] = color[i];
    s_cfgHamWidthMin = hamidashiWidthMin;
    s_cfgValid = true;
    s_cfgDirty = true;
}
...
int lc_formatDebug(char* buf, int bufLen) {
    ...
    int o = snprintf(buf, bufLen,
        "[LC] n=%d in=%d fo=%d steer=%d act=%d lock=%d prog=%d sn=%d oc=%d g=%d ab=%u as=%d err=%d p=",
        s_dbgLc.count, s_dbgCls.inIndex, s_dbgFo, s_dbgSteer,
        s_dbgCommitActive ? 1 : 0, s_dbgCommitLocked ? 1 : 0, s_dbgCommitProgress,
        s_cSeen ? 1 : 0, s_dbgCls.outCount, s_dbgGreen,
    ...
    for (int i = 0; i < nshow && o < bufLen - 16; i++) {
        ...
        o += snprintf(buf + o, bufLen - o, "%s%d,%d,%c,%d",
            ...
            role, s_dbgLc.crossings[i].width);
    }
    if (o < bufLen - 1) { buf[o++] = '\n'; buf[o] = '\0'; }
    return o;
}
...
int lc_formatConfig(char* buf, int bufLen) {
    if (!s_cfgValid || bufLen < 96) return 0;
    const uint32_t now = millis();
    if (!s_cfgDirty && (uint32_t)(now - s_cfgLastSentMs) < 2000UL) return 0;
    const int n = snprintf(buf, bufLen,
        "[CFG] arc=%d,%d,%d,%d,%d,%d,%d,%d,%d,%d silver=%d,%d,%d,%d color=%d,%d,%d hmin=%d\n",
        s_cfgArc[0], s_cfgArc[1], s_cfgArc[2], s_cfgArc[3], s_cfgArc[4],
        s_cfgArc[5], s_cfgArc[6], s_cfgArc[7], s_cfgArc[8], s_cfgArc[9],
        s_cfgSilver[0], s_cfgSilver[1], s_cfgSilver[2], s_cfgSilver[3],
        s_cfgColor[0], s_cfgColor[1], s_cfgColor[2], s_cfgHamWidthMin);
