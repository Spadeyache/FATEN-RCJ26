# Pure-python YOLOv8 anchor-free decoder shared by det_image_yolov8.py and
# det_live_xy_yolov8.py.  Drop in /data/k230-train/ next to the two main
# scripts so they can `from yolov8_decode import ...`.
#
...
#   are cx,cy,w,h in MODEL letterbox pixel coords and channels 4..3+nc
#   are sigmoided class scores.  Feeding that to aicube reads bbox
#   channels as scores and prints things like "silver 638.50".
...
def decode_yolov8_anchorfree(flat_out, num_classes, conf_thr,
    ...
    The kmodel was compiled with output_layout=NCHW so the flattened
    buffer is channel-major: flat[c * N + n] = channel c, anchor n.
