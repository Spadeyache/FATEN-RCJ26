...
def parse_times(path: str) -> List[List[float]]:
    with open(path, 'r') as f:
        lines: List[str] = f.readlines()
        times = []
        for line in lines:
            items = [float(i.strip()) for i in line.split('|')[2:-1]]
            times.append([items[0],items[1],items[2]])
        return times
...
if __name__ == '__main__':
    ...
    2. python draw_trace.py --local_trace 1 #(local_trace = 1)
        Move 10_FusionToTirPass/mac.csv, 13_DumpTraceInfoPass/*.py and k230_new_arch_kpu_time.txt together.
    '''
    parser = argparse.ArgumentParser(description='Control local_trace execution path.')
    parser.add_argument('--local_trace', type=int, default=0, help='Set local_trace to 0 or 1')
    args = parser.parse_args()
    local_trace = args.local_trace
    if not local_trace:
        ...
        if os.path.exists(kpu_times_file):
            kpu_times = parse_times(kpu_times_file)
            i = 0
            for item in v0.data:
                item.begin = kpu_times[i][0]
                item.end = kpu_times[i][1]
                item.exec_time = kpu_times[i][2]
                i += 1
        else:
