...
Usage:
    python3 test_kmodel.py <model.kmodel> <model.onnx> <images_dir>
        [--conf 0.05] [--limit 20] [--img-height 480] [--img-width 640]
        [--labels black silver]

Output: side-by-side table of float vs kmodel confidence per image and per class.
"""
...
# Match on-device CanMV settings from Yolov8nrun2.py
#   confidence_threshold = 0.03
#   nms_threshold        = 0.15
#   model_input_size     = [640, 480]   (W, H)
#   rgb888p_size         = [640, 480]   (no letterbox, direct resize)
#   labels               = ["black", "silver"]
CONF_DEFAULT = 0.03
IOU_THRESH = 0.15
MAX_BOXES = 50
...
def postprocess(output, conf_thresh, img_height, img_width):
    ...
    # Mirror on-device behavior: NMS, then cap to max boxes
    return nms(dets, IOU_THRESH)[:MAX_BOXES]
...
class KModelSim:
    ...
    The simulator handles mean/std division internally because preprocess=True
...
def main():
    ...
    for fp in files:
        ...
        for (cls_id, cf), (_, ck) in zip(best_f, best_k):
            label = args.labels[cls_id] if cls_id < len(args.labels) else f"c{cls_id}"
            delta = ck - cf
            deltas.append(delta)
            marker = "  WARN" if delta < -0.2 else ""
    ...
    if deltas:
        ...
        print("=" * 70)
        print(f"Delta summary over {len(deltas)} (image,class) pairs:")
        ...
        print()
        if deltas.mean() > -0.05:
            print("OK Quantization looks healthy.")
        elif deltas.mean() > -0.15:
            print("~  Moderate quant loss - try --w-quant-type int8, --calib-method NoClip.")
        else:
            print("X  Heavy quant loss - calibration data is likely off-distribution.")
