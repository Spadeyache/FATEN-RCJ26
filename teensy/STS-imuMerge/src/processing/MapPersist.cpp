...
namespace {
    // In-RAM shadow of the most-recently-saved semantic array. Diff against
    // this to minimise EEPROM writes.
    ...
    uint32_t crc32_compute_eeprom(uint16_t addr, uint16_t length) {
        ...
        for (uint16_t i = 0; i < length; ++i) crc = crc32_update(crc, EEPROM.read(addr + i));
...
uint16_t mapPersistDirtyCount(const EvacMap& m) {
    uint16_t n = 0;
    for (uint16_t i = 0; i < MAP_N; ++i) if (m.semantic[i] != _shadow[i]) ++n;
    return n;
}
