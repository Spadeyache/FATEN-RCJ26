...
# Decoder is INLINED below (see _iou, decode_yolov8_anchorfree, ...).
# CanMV MicroPython doesn't expose sys.path, so a sibling-module import
# ("from yolov8_decode import ...") fails with
# "AttributeError: 'module' object has no attribute 'path'".
# Inlined version is the only reliable layout on this firmware.
...
# ---------------------------------------------------------------------------
# Inlined YOLOv8 anchor-free decoder. Identical to yolov8_decode.py contents.
# Inlined because CanMV MicroPython lacks sys.path.
# ---------------------------------------------------------------------------
def _iou(a, b):
...
def decode_yolov8_anchorfree(flat_out, num_classes, conf_thr,
                              num_anchors=NUM_ANCHORS_640x480):
    """Decode YOLOv8 single concatenated output (1, 4+nc, N).

    Channel-major NCHW: flat[c * N + n] = channel c, anchor n.
    Channels 0..3 = cx, cy, w, h in MODEL letterbox px coords.
    Channels 4..3+nc = sigmoided class scores.
