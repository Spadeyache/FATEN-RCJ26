...
cpu_ops = [
    ...
    ['Squeeze', dict()],
    ['Reshape', dict()],
    ['GetItem', dict(OffChipMemoryTraffic=1612800)],
    ['Squeeze', dict()],
    ['GetItem', dict(OffChipMemoryTraffic=50400)],
    ['Squeeze', dict()],
    ['GetItem', dict(OffChipMemoryTraffic=1612800)],
    ['Softmax', dict(OffChipMemoryTraffic=3225600,FLOPs=14918402,Parallel=4)],
    ['Slice', dict(OffChipMemoryTraffic=201600)],
    ['Binary', dict(OffChipMemoryTraffic=151200,FLOPs=12600,Parallel=4)],
    ['Reshape', dict()],
    ['Slice', dict(OffChipMemoryTraffic=201600)],
    ['Reshape', dict()],
    ['Binary', dict(OffChipMemoryTraffic=151200,FLOPs=12600,Parallel=4)],
...
if __name__ == '__main__':
    ...
    for item in roofline.info.data:
        ...
        mac_cpu = np.mean([e[1] / e[0] for e in cpu_entrys]) if cpu_entrys else 0.0
    mac_kpu = np.mean([e[1] / e[0] for e in kpu_entrys]) if kpu_entrys else 0.0
    all_entrys = cpu_entrys + kpu_entrys
    mac_kpu_cpu = np.mean([e[1] / e[0] for e in all_entrys]) if all_entrys else 0.0

    print(f' Mac usage cpu : {mac_cpu:.3f}\n Mac usage kpu : {mac_kpu:.3f}\n Mac usage kpu and cpu : {mac_kpu_cpu:.3f}\n Estimate Exec Time: {es_time:.3f} ms\n Fps : {1000 / es_time:.3f}')
